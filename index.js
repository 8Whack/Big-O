...
// ANSWER:
function fourth(numbers) {
  return numbers[3]
}
// O(1) since it's always going to grab the 4th item in the array, independent of the size of the array.
...
// this has O(n), since it doesn't create a second array.
...
// ANSWER: for both, the information is there only once, and while it's being modified, it's not being stored just changed and the only variable is the length. so its jest based on the input.
